...
describe('AppController', () => {
  ...
  describe('verifyJWT', () => {
    it('should return "verdadeiro" upon submit of correct token', () => {
      const token =
        'eyJhbGciOiJIUzI1NiJ9.eyJSb2xlIjoiQWRtaW4iLCJTZWVkIjoiNzg0MSIsIk5hbWUiOiJUb25pbmhvIEFyYXVqbyJ9.QY05sIjtrcJnP533kQNk8QXcaleJ1Q01jWY_ZzIZuAg';
      expect(appController.verifyJWT(token)).toBe('verdadeiro');
    });

    it('should return "falso" upon submit of invalid token', () => {
      const token =
        'eyJhbGciOiJzI1NiJ9.dfsdfsfryJSr2xrIjoiQWRtaW4iLCJTZrkIjoiNzg0MSIsIk5hbrUiOiJUb25pbmhvIEFyYXVqbyJ9.QY05fsdfsIjtrcJnP533kQNk8QXcaleJ1Q01jWY_ZzIZuAg';
      expect(appController.verifyJWT(token)).toBe('falso');
    });

    it('should return "falso" upon submit of invalid claim name, containing numbers', () => {
      const token =
        'eyJhbGciOiJIUzI1NiJ9.eyJSb2xlIjoiRXh0ZXJuYWwiLCJTZWVkIjoiODgwMzciLCJOYW1lIjoiTTRyaWEgT2xpdmlhIn0.6YD73XWZYQSSMDf6H0i3-kylz1-TY_Yt6h1cV2Ku-Qs';
      expect(appController.verifyJWT(token)).toBe('falso');
    });

    it('should return "falso" upon submit of invalid token, containing more then 3 claims', () => {
      const token =
